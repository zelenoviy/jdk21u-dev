...
import jdk.internal.org.jline.terminal.impl.jna.haiku.HaikuNativePty;
...
class JDKNativePty {
    ...
    static JnaNativePty current(TerminalProvider provider, SystemStream systemStream) throws IOException {
        return HaikuNativePty.current(provider, systemStream);
    }

    static JnaNativePty open(TerminalProvider provider, Attributes attr, Size size) throws IOException {
        return HaikuNativePty.open(provider, attr, size);
    }

    static int isatty(int fd) {
        return HaikuNativePty.isatty(fd);
    }

    static String ttyname(int fd) {
        return HaikuNativePty.ttyname(fd);
    }
